...
struct zo_<?=$class_name?> 
{
    ...
    <?=$extension?>_object_type object_type;
...
#ifdef <?=strtoupper($extension)?>_INCLUDE_METHOD_TABLES
...
<?if(isset($defClassProperties[$class_name]) && isset($defIni[$class_name][$class_name])){?>
	PHP_ME(php_<?=$class_name?>, __get, <?=$extension?>_<?=$file_name?>_get_args, ZEND_ACC_PUBLIC)
<?}?>
	PHP_FE_END
};
#endif
